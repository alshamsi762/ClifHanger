...
const gSG = require('../_gameStage/_frontGlobal.js');
const gSM = require('../_gameStage/_frontMain.js');
const gSB = require('../_gameStage/_frontBuilder.js');
const FP = require('./_fallPiece.js');
const LAMN = require('./_lookAtMeNow.js');
const MP = require('./_movePiece.js');
/** fallPiece **/

test('Test if object to fall have been defined', async () =>{
  gSB.buildStage(5);

  gSG.obj3DArray[1].position.z = 10

  await FP.fallPiece(1);

  await expect(gSG.obj3DArray[1].position.z).toBe(0);
});
test('Test if object to fall have been defined', async () =>{
  gSB.buildStage(5);

  //make copy of old object
  let oldObj = Object.assign({}, gSB.obj3DArray[1]);

  console.log(oldObj.position.x);

  MP.movePiece(1,"X+");

  console.log(gSG.obj3DArray[1].position.x);

  expect(gSG.obj3DArray[1].position.x).toBe(5);
});
